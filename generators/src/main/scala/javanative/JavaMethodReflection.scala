...
object JavaMethodReflection__Java_1_8 extends JavaJMHProjectBuilder {
  ...
  object Syntax extends RecordSyntax {
    ...
    def create(fields: Seq[(String, String)]): String = {
      ...
      if (idx == -1){
        // e.g. GetterClass4(1,2,3,4)
        fields.map{ case (k, v) => s"""$v""" }.mkString(s"new GetterClass${fields.length}(", ", ",")")
      }else{
        fields.map{ case (k, v) => s"""$v""" }.mkString(s"new GetterClassPoly${fields.length}_${idx+1}(", ", ",")")
      }
    }
