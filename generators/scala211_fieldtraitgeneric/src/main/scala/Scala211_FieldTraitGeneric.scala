...
object Scala211_FieldTraitGeneric extends BenchmarkAndLibraryGenerator with Scala211Language {

  val name = "scala211_fieldtraitgeneric"

  lazy val library = new RecordLibrary {
    val name = s"scala211_fieldtraitgeneric_lib"

    val pkg = List("se", "obkson", "wreckage", name)

    val output = Dependency(List("se", "obkson", "wreckage"), s"${name}_2.11", "0.1")
